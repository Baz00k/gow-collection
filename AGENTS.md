...
`images/base/` is a Fedora-based image providing the shared runtime contract (runtime user + `gosu` handoff, `/etc/cont-init.d/*` runner, NVIDIA/device init, non-setuid bubblewrap for Flatpak sandboxing and PID-sharing portals). It owns the `ENTRYPOINT` (`/opt/gow/entrypoint.sh`). App images `FROM ${BASE_APP_IMAGE}` and only add packages + an overlay `startup.sh`.
