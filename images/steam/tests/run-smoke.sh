...
log_header "Steam Image - Smoke Test Suite"
echo "Image: ${IMAGE_NAME}"
echo "Timestamp: $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
echo ""
...
if [[ "${SKIP_BUILD}" == "true" ]]; then
    ...
    log_test_skip "Build test (not applicable for steam)"
    echo "1. BUILD: SKIPPED" >> "${EVIDENCE_FILE}"
fi
...
TESTS_TOTAL=$((TESTS_TOTAL + 1))
log_test_start "Steam Compatibility"
if "${SCRIPT_DIR}/smoke-steam-compat.sh"; then
    log_test_pass "Steam Compatibility"
    echo "4. STEAM-COMPAT: PASSED" >> "${EVIDENCE_FILE}"
else
    log_test_fail "Steam Compatibility"
